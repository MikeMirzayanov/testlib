...
const size_t INPUT_STREAM_BUFFER_SIZE = 1000000;
const size_t INPUT_STREAM_MAX_UNREAD_CHAR_COUNT = 1000;
...
static bool __testlib_refill(InStream* stream)
{
    ...
    if (stream->bufferPos >= int(stream->bufferSize))
    {
        ...
        stream->bufferSize = INPUT_STREAM_MAX_UNREAD_CHAR_COUNT + size_t(readSize >= 0 ? readSize : 0);
